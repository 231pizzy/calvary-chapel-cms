...
export default function SingleView({ editUrl, submitEndpoint, returnUrl }) {
    ...
    useEffect(() => {
        getRequestHandler({
            route: `/api/contact-email-view?id=${id}`,
    ...
    return <Box sx={{ width: '100%' }}>
        <BackButton title={'View Contact Enquiry Email'} components={
            [
                <Edit url={`${editUrl}?id=${id}`} />,
                <Delete title={'Contact Enquiry Email'}
                    returnUrl={`/admin/contact/contact-email`}
                    id={id}
        ...
        {showForm ?
            <Box sx={{
                ...
                {/* Title */}
                <DataItem label={'Email Address'} value={data?.title} />
            </Box> : <Loader />
        }
    </Box >
}