...
export default function SingleView({ editUrl, submitEndpoint, returnUrl }) {
    ...
    useEffect(() => {
        getRequestHandler({
            route: `/api/prayer-request-topic-view?id=${id}`,
    ...
    return <Box sx={{ width: '100%' }}>
        <BackButton title={'View Contact Form Option'} components={
            [
                <Edit url={`${editUrl}?id=${id}`} />,
                <Delete title={'Contact Form Option'}
                    returnUrl={`/admin/contact/prayer-request-topics`}
                    id={id}
