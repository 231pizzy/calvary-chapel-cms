...
export default function AddView({ submitEndpoint, returnUrl }) {
    ...
    useEffect(() => {
        ...
        id && fetch(('/api/prayer-request-topic-view' + (id ? `?id=${id}` : '')), { method: 'GET' }).then(
            async response => {
    ...
    return <Box sx={{ width: '100%' }}>
        <BackButton title={id ? 'Edit Contact Form Option' : 'Create New Contact Form Option'} />
                        ...
                        return (<Form style={{ width: '100%', marginBottom: '36px' }}>
                            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
                                ...
                                <Box sx={{ mb: 2, width: '100%' }}>
                                    ...
                                    {/* Textfield */}
                                    <FormTextField placeholder={'Eg. Salvation'} name='title' />
                                </Box>
        ...
        {saved && <ModalMessage
            title={id ? 'Contact Form Option Updated' : 'Contact Form Option Created'} open={saved} handleCancel={closeSuccessAlert} type='success'
            message={`You have just ${id ? 'updated this' : 'created a new'} contact form option`} />}
