...
export default function Preview({ handleClose, handleSubmit, allowPublish, formProps }) {
    ...
    const getImage = (id) => {
        return imageFromFile({ file: Object.values((formData[id] ?? [{}])[0])[0] })
    }
    ...
    return <Modal open onClose={handleClose} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Box sx={{ width: { xs: '90%', md: '80%' }, m: 'auto', bgcolor: 'white' }}>
            ...
            <Box sx={{
                ...
                {/* Publish button */}
                {allowPublish && <SubmitButton fullWidth={false} marginTop={0} label={'Publish'} formProps={formProps} />}
            </Box>

            {/* Content */}
            <IndexView preview={true}
                banner={getImage('banner')} heroText={formData?.heroText}
                heroSubtitle={formData?.heroSubtitle}
                bodyImage={getImage('bodyImage')} bodyTitle={formData?.bodyTitle}
