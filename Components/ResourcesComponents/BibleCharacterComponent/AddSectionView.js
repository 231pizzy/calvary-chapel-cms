...
export default function AddSectionView({ closeForm, sectionName, id, submitEndpoint, returnUrl, pageName }) {
    const [initialValues, setInitialValues] = useState({
        name: sectionName ?? ''
    })
    const [validationSchema, setValidationSchema] = useState({
        name: Yup.string().required("section's name is required"),
    })
        ...
        await postRequestHandler2({
            ...
            successCallback: body => {
                ...
                if (result) {
                    ...
                    message = 'Bible Character Section Updated'
                ...
                <Form style={{ width: '100%', marginBottom: '36px' }}>
                    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
                        ...
                        <Box sx={{ mb: 2, width: '100%' }}>
                            ...
                            {/* Textfield */}
                            <FormTextField placeholder={'Section Name'} name='name' />
                        </Box>
