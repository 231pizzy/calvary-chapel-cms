...
export default function AddSectionView({ closeForm, submitEndpoint, sectionName, id, returnUrl, pageName }) {
    const [initialValues, setInitialValues] = useState({
        name: sectionName ?? ''
    })
    const [validationSchema, setValidationSchema] = useState({
        name: Yup.string().required("The name of the conference is required"),
    })
        ...
        await postRequestHandler2({
            ...
            successCallback: body => {
                ...
                if (result) {
                    ...
                    message = 'Conference Section Saved'
                ...
                <Form style={{ width: '100%', marginBottom: '36px' }}>
                    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
                        ...
                        <Box sx={{ mb: 2, width: '100%' }}>
                            {/* Textfield */}
                            <FormTextField placeholder={'Conference name'} name='name' />
                        </Box>
