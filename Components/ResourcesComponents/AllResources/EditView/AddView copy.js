...
export default function AddNewResourceView({ submitEndpoint, returnUrl }) {
            ...
            async response => {
                ...
                if (data) {
                    ...
                    result?.conferences?.filter(i => i?.name).forEach(i => {
                        ...
                        /*  return {
                             value: i?._id, label: i?.name, parent: i?.sectionTitle
                         } */
                    })
    ...
    const handleFormSubmit = async (value) => {
        ...
        const audio = body.audioDownload
        const document = body.documentDownload
        const updateAudio = (audio instanceof Blob);
        const updateDocument = (document instanceof Blob);

        const fileArray = [];
        if (updateAudio) {
            const filedata = getFileData({ file: audio, folder: 'audio' })
            fileArray.push(filedata);
            body.audioDownload = filedata.filename
        };
        if (updateDocument) {
            const filedata = getFileData({ file: document, folder: 'document' })
            fileArray.push(filedata);
            body.documentDownload = filedata.filename
        };

        console.log('file array to upload', fileArray);
        if (fileArray?.length) {
            await uploadFile({ fileArray, callback })
        }
        else {
            await callback()
        }
                            ...
                            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
                                ...
                                {formProps.values.scripture && formProps.values.bookOfScripture && <Box sx={{ mb: 2, width: '100%', display: 'flex', alignItems: 'flex-start' }}>
                                    ...
                                    <Box sx={{ mb: 2, width: 'max-content', mr: 3 }}>
                                        ...
                                        {/* Textfield */}
                                        <FormTextField noValidation={true} min={1} type={'number'} small={true} placeholder={''} name='chapter' />
                                    ...
                                    <Box sx={{ mb: 2, width: 'max-content' }}>
                                        ...
                                        <Box sx={{ display: 'flex', alignItems: 'flex-start', width: 'max-content' }}>
                                            ...
                                            <FormTextField noValidation={true} min={1} type={'number'} small={true} placeholder={''} name='verse.start' />
                                            ...
                                            {/* End verse*/}
                                            <FormTextField noValidation={true} min={1} type={'number'} small={true} placeholder={''} name='verse.end' />
                                        </Box>
