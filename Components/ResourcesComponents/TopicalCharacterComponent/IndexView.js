...
export default function IndexView({ editUrl, preview, previewPayload, handleOpenCreateForm, handleCloseCreateForm, setCount, showAddNewForm }) {
    ...
    const handleDeleteItem = async () => {
        setDeleting(true);
        const response = await fetch(`/api/delete-topical-study?id=${showItemDeleteWarning?.id}`, { method: 'GET' });
    ...
    return (data ?
        <Box sx={{
            ...
            {showAddNewForm && <WarningModal open={showAddNewForm} showAction={false}
                handleCancel={handleCloseCreateForm} title='Add New Topical Study' type="success"
                message={<AddTopicView closeForm={handleCloseCreateForm} id={openItemForm}
                    submitEndpoint={'/api/add-topical-study'} name={itemName}
                    pageName={'topicalStudy'} />}
            />}

            {showItemDeleteWarning && <WarningModal open={showItemDeleteWarning}
                handleCancel={handleCloseItemDeleteWarning} title='Delete Topical Study' type="success"
                message={`You are about to delete this  topical study`} proceedAction={handleDeleteItem}
